...
main(int argc, char **argv)
{
  ...
  if (argc != 3) {
    cerr << "usage: total_etc_2 number-of-elements seed\n";
    exit(1);
  }

  ss.str(argv[1]);
  if (!(ss >> n)) { cerr << "usage: total_etc_2 number-of-elements seed\n"; exit(1); }

  ss.clear(); ss.str(argv[2]);
  if (!(ss >> seed)) { cerr << "usage: total_etc_2 number-of-elements seed\n"; exit(1); }
